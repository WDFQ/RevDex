...
export default function LoginScreen() {
    ...
    async function handleContinue() {
        // disallow button click if already submitting
        if (isSubmitting) {
            return
        }

        // start the button click
        ...
        try {
            ...
            if (err instanceof FirebaseError) {
                if (err.code === 'auth/invalid-credential' || err.code === 'auth/wrong-password' || err.code === 'auth/user-not-found') {
                    ...
                    Alert.alert('Too many attempts', 'Access temporarily disabled due to too many failed attempts. Try again later.')
                } else {
            ...
            <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
                <View className="flex-1 justify-center px-6">
                    ...
                    <View className="items-center mb-8">
                        ...
                        <Text className="text-neutral-400 text-sm text-center leading-5 max-w-xs">Log in to pick up the hunt and keep building your digital garage.</Text>
                    </View>
                    ...
                    {/* Primary action */}
                    <TouchableOpacity activeOpacity={0.85} className="bg-sky-200 rounded-2xl py-4 items-center mb-3" onPress={() => handleContinue()} disabled={isSubmitting}>
                        <Text className="text-sky-900 text-base font-semibold">{isSubmitting ? 'Please wait...' : 'Continue'}</Text>
                    </TouchableOpacity>
